...
\$accent = rgb($(grep '^accent=' "$PALETTE" | head -1 | cut -d= -f2 || echo "${C[blue]}"))
\$accent_secondary = rgb($(grep '^accent=' "$PALETTE" | head -1 | cut -d= -f2 || echo "${C[blue]}"))
\$border_inactive = rgba(${C[bright_black]}66)"
echo "$hypr_colors" > "$HOME/.config/hypr/UserConfigs/monochrome-colors.conf"
echo "$hypr_colors" > "$HOME/.config/hypr/UserConfigs/aurora-colors.conf"
